...
if(isset($_POST['page'])){
    ...
    //get number of rows
    $queryNum = $conn->query("SELECT COUNT(*) as postNum FROM purchase_mst where usage_flag = '0' and c_id = '".$_SESSION['company_id']."' ".$whereSQL.$orderSQL);
    ...
    //get rows
    $query = $conn->query("SELECT * FROM purchase_mst where usage_flag = '0' and c_id = '".$_SESSION['company_id']."' $whereSQL $orderSQL LIMIT $start,$limit");
			...
			<table id="" class="table table-bordered table-hover">
				...
				<tbody>
					<?php while($row = $query->fetch_assoc()){ $postID = $row['s_id']; $account = $con->query("select * from client_master where client_id = '".$row['act_no']."'")->fetch_object(); $prj_name = $con->query("select * from project_master where project_id = '".$row['prj_id']."'")->fetch_object();?>
						<tr>
						<td style="text-align:center;"><a href="pu_invoice.php?id=<?php echo $row['s_id']; ?>" ><button type="button" class="btn btn-default btn-sm">View</button></a> &nbsp;<a href="edit_pu.php?id=<?php echo $row['s_id']; ?>"><button type="button" class="btn btn-info btn-sm">Edit</button></a> &nbsp;<a href="Process/delete.php?purchase_invoice_del=<?php echo $row['s_id']; ?>" onclick="return confirm('Are you sure you want to delete this Invoice ?');"><button type="button" class="btn btn-danger btn-sm">Delete</button></a></td>
