...
global $data;
global $type , $client;
$data = $conn->query("SELECT * FROM `sales_return_mst` WHERE id = '".$_GET['id']."'")->fetch_object();
...
function Header()
{
	...
	$this->Cell(70,8,'Sales Return Invoice',0,1,'C'); 
	$this->SetX(140);
	$this->SetFont('Arial','',12);
	$this->SetX(7);
	$this->SetFont('Arial','',12);
...
$product = $conn->query("select * from sales_return_detail_mst where s_id = '".$_GET['id']."'");
$c = 0; $t = 0;
...
$pdf->Output("Sales Return ".$data->pr_no.".pdf","D");
?>