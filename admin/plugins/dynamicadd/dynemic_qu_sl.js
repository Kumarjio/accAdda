...
$('#sales_detail').focus(function(){
	...
	{
		$('#pr_name').html('Please Select Account Name First');
	}
	else
	{
		$('#pr_name').html('');
		$( "#sales_detail" ).autocomplete({
			  source: 'search/sales_detail.php',
				select:function(e, ui){
					...
					$('#id').val(ui.item.id);
					$('#hsn').val(ui.item.hsn);
					$('#sales_detail1_grate').val(ui.item.gst);
	...
	$('#sales_detail').click(function(){
		$("#sales_detail").removeAttr('readonly');
		$("#sales_detail").val('');
	});
	$('#sales_total1_quentity').focus( check );
	$('#sales_total1').focus( function(){
		if( $('.1d').val() == '' )
		{
			$("#qty_nm").html('Please Select Product Name');
		}
...
$('#sales_total1').keyup( Toprice );
$('#sales_total1_quentity').keyup( Toprice1 );
