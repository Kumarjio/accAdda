...
if(isset($_POST['page'])){
    ...
    //get number of rows
    $queryNum = $conn->query("SELECT COUNT(*) as postNum FROM reciept_mst where `df` = '0' and c_id = '".$_SESSION['company_id']."' ".$whereSQL.$orderSQL);
    ...
    //get rows
    $query = $conn->query("SELECT * FROM reciept_mst where `df` = '0' and c_id = '".$_SESSION['company_id']."' $whereSQL $orderSQL LIMIT $start,$limit");
			...
			<table id="" class="table table-bordered table-hover">
				...
				<tbody>
					...
					<?php $act_name=$con->query("SELECT * FROM `client_master` WHERE client_id = '".$row['act_id']."'")->fetch_object();?>
						<tr>
							<td style="text-align:center;"><a href="view_rec_fi.php?Rid=<?php echo $row['id']; ?>" ><button type="button" class="btn btn-default btn-sm">View</button></a> &nbsp;<a href="Process/delete.php?del_receipt_id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this Detail ?');"><button type="button" class="btn btn-danger btn-sm">Delete</button></a></td>
