...
$add_chalan = $conn->query("UPDATE `n_quat_mst` SET 
...
$del = $conn->query("DELETE FROM `n_quat_detail_mst` WHERE s_id = '".$_POST["s_id_u"]."' ");
...
foreach($_POST['product_name'] as $index => $val) {
	...
	{
		header('location:../qu_invoice.php?id='.$_POST["s_id_u"]);
		exit;
	}
?>