...
<?php $sel_pur_no1 = $conn->query("SELECT * FROM purchase_return_mst ORDER BY id DESC LIMIT 1")->fetch_object();?>
<?php 
if( !isset($_GET['invoice_id_sel'])){ echo "<script>window.location='return_pu_sel.php';</script>"; }
$sels = $conn->query("select * from purchase_mst where s_id = '".$_GET['invoice_id_sel']."'")->fetch_object();
$client = $con->query("select * from client_master where client_id = '".$sels->act_no."'")->fetch_object();
$product = $conn->query("select * from purchase_detail_mst where s_id = '".$_GET['invoice_id_sel']."'");
?>
	...
	<form action="Process/purchase_return_invoice.php" enctype="multipart/form-data" method="post">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="col-md-6">
								...
								<input type="text" class="form-control" value="PR_<?php if(empty($sel_pur_no1->id)){echo 1;} else {echo $sel_pur_no1->id+1;} ?>" id="" name="s_number" placeholder="sales return nmuber" readonly>
							...
							<label>Project Name</label>
							    <select id="re_sales_invoice_pro" name="project_name" class="form-control" required>
									...
									<?php while($sel_projectsr = $sel_projects->fetch_object()){ ?>
										<option value="<?php echo $sel_projectsr->project_id; ?>" <?php if( $sels->prj_id == $sel_projectsr->project_id){ echo "selected"; } ?>><?php echo $sel_projectsr->project_name; ?></option>
									<?php } ?>
								</select>
						</div>
						</div>
				...
				<table id="table" class="table table-bordered table-hover">
						...
						<?php $c = 0; while($productr = $product->fetch_object()){ $c++; ?>
							<tr id="<?php echo $c; ?>">
								...
								<td><input type="number" min="0" style="width:70px;" name="quntity[]" class="<?php echo $c; ?>" value="<?php echo $productr->qty; ?>" id="sales_total<?php echo $c; ?>_quentity" required/></td>
								<td><input type="number" min="0" style="width:70px;" name="price[]" value="<?php echo $productr->rate; ?>" id="sales_total<?php echo $c; ?>" required/></td>
					...
					<div class="col-md-6">
						<div class="form-group">
							<label for="">Freight</label>
								<input type="number" class="form-control" min="0" id="freight" name="freight" placeholder="Enter Freight" value="0" required>
						</div>
					</div>
					<div class="col-md-6">
					<div class="form-group">
