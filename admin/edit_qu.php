...
$chalan = $conn->query("SELECT * FROM n_quat_mst where s_id = '".$_GET['id']."'")->fetch_object();
$client = $con->query("Select * from client_master where client_id = '".$chalan->act_no."'")->fetch_object();
?>
<form action="Process/edit_qu_process.php" id="form_full" method="post"> 
				...
				<div class="box-header with-border">
					...
					<div class="col-md-6">
								<div class="form-group">
								  ...
								  <input type="text" class="form-control" id="" value="<?php echo $chalan->po_no; ?>" name="po_no" placeholder="Enter Po No.">
						...
						<div class="form-group">
							<label>PO Date </label>
								<div class="input-group">
									...
									<input type="text" placeholder="Select PO Date" name="po_date" value="<?php echo $chalan->po_date; ?>" id="product_po_date_sales" class="form-control">
								</div>
								<script> 
								$(function()
								{
					...
					<div class="col-md-6">
						<div class="form-group">
						  ...
						  <input type="text" class="form-control" value="<?php echo $chalan->lr_no; ?>" id="" name="lr_no" placeholder="Enter Lr No.">
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group">
						...
						<div class="form-group">
							<label>LR Date </label>
								<div class="input-group">
									...
									<input type="text" placeholder="Select LR Date" value="<?php echo $chalan->lr_date ?>" name="lr_date" id="product_lr_date_sales" class="form-control">
								</div>
								<script> 
								$(function()
								{
		...
		<div class="col-md-12">
			...
			<div class="box-header with-border">
				...
				<table id="table" class="table table-bordered table-hover">
						...
						<?php $p_de = $conn->query("select * from n_quat_detail_mst where s_id = '".$_GET['id']."'"); $c = 0; $su_total = 0; $cgst_s = 0; $sgst_s = 0; $igst_s = 0;
						while($p_der = $p_de->fetch_object()){ $c++;
						?>
							<tr id="<?php echo $c; ?>">
								<td><?php echo $c; ?><input type="hidden" class="<?php echo $c; ?>d" name="sr_no_product[]" value="1" /><input type="hidden" name="" id="rem<?php echo $c; ?>"  value="<?php echo $p_der->sd_id; ?>" /></td>
								...
								<td><input type="number" min="0" style="width:70px;" name="quntity[]" value="<?php echo $p_der->qty; ?>" class="<?php echo $c; ?>" id="sales_total<?php echo $c; ?>_quentity" required ></td>
								<td><input type="number" min="0" style="width:70px;" name="price[]" value="<?php echo $p_der->rate; ?>" id="sales_total<?php echo $c; ?>" required/></td>
		...
		<div class="box box-primary">
					...
					<div class="col-md-6">
						<div class="form-group">
							<label for="">Freight</label>
								<input type="number" min="0" class="form-control" value="<?php echo $chalan->freight; ?>" id="" name="freight" placeholder="Enter Freight" value="0">
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group">
			...
			<div class="box-body">
						  <div class="box-footer">
							<button type="submit" name="submit" onclick="return auth()" class="btn btn-primary">Update</button>
...
function auth()
{
	if($('#account_name_sales').val() != '')
	{
		...
		for( j = 1; j <= tab; j++ )
		{
			...
			{
				$('#errall').fadeIn("slow");
				$('#errall').html("Please Add Product Name");
				return false;
			}
		}
		for(i = 1; i <= tab; i++)
...
<script src="plugins/dynamicadd/dynemic_qu_sledit.js"></script>
