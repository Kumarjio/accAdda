...
$data = $conn->query("SELECT * FROM `purchase_mst` WHERE s_id = '".$_GET['id']."'")->fetch_object();
...
function Header()
{
	...
	$this->Cell(70,8,'PURCHASE INVOICE',0,0,'C'); 
	$this->SetX(140);
	$this->SetFont('Arial','',12);
...
$pdf = new PDF();
$pdf->AliasNbPages();
	$result = $conn->query("SELECT COUNT(*) as row FROM purchase_detail_mst WHERE s_id = '".$data->s_id."'");
...
if( $r->row > 5 )
{
	...
	{   
		$pdf->AddPage();
		$start = ($i - 1) * $limit; 
		$detail = $conn->query("SELECT * FROM `purchase_detail_mst` WHERE s_id = '".$data->s_id."' ORDER BY sd_id limit ".$start.",".$limit);
	...
	$pdf->SetXY(183,63);
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(20,5,$data->ch_date,0,'L');
	...
	$sr = 1; $c = 5; while($detailr = $detail->fetch_object())
	{
		...
		$pdf->MultiCell(31,5,$data->remark,'','L');
			...
			$pdf->SetFont('Arial','',10);
			$pdf->Cell(1,20,'','R',0,'C');
			$detaila = $conn->query("SELECT * FROM `purchase_detail_mst` WHERE s_id = '".$data->s_id."' ORDER BY sd_id limit ".$start.",".$limit);
			$ac = 0;
			$tval = 0; $tax = 0;
...
{
	$detail = $conn->query("SELECT * FROM `purchase_detail_mst` WHERE s_id = '".$data->s_id."'");
$pdf->AddPage();
$pdf->SetFont('Times','',12);
	$pdf->SetX(7);
	$pdf->SetFont('Arial','',12);
	...
	$sr = 1; $c = 5; while($detailr = $detail->fetch_object())
	{
		...
		$pdf->MultiCell(31,5,$data->remark,'','L');
			...
			$pdf->SetFont('Arial','',10);
			$pdf->Cell(1,20,'','R',0,'C');
			$detaila = $conn->query("SELECT * FROM `purchase_detail_mst` WHERE s_id = '".$data->s_id."'");
			$ac = 0;
			$tval = 0; $tax = 0;
		while($detailar = $detaila->fetch_object())
			{
...
$pdf->Output("Purchase - ".$data->s_numner.".pdf", 'D');
?>