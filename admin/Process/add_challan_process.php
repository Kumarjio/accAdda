...
$add_chalan = $conn->query("INSERT INTO `quatation_mst`(`s_numner`, `act_no`, `prj_id`, `s_date`, `po_no`, `po_date`, `transport`, `lr_no`, `lr_date`, `vat_no`, `cst_no`, 
...
foreach($_POST['product_name'] as $index => $val) {
	...
	{		if( !empty($client_detail->client_detail) )		{			e_mail( $_POST["id_purchase_invoice"] );		}
		header('location:../chalan_invoice.php?id='.$_POST["id_purchase_invoice"]);
		exit;
	}		function e_mail($id)	{		include('../fpdf/qmail.php?id='.$id);	}
?>