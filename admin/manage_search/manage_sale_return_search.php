...
if(isset($_POST['page'])){
    ...
    //get number of rows
    $queryNum = $conn->query("SELECT COUNT(*) as postNum FROM sales_return_mst where df = '0' and c_id = '".$_SESSION['company_id']."' ".$whereSQL.$orderSQL);
    ...
    //get rows
    $query = $conn->query("SELECT * FROM sales_return_mst where df = '0' and c_id = '".$_SESSION['company_id']."' $whereSQL $orderSQL LIMIT $start,$limit");
			...
			<table id="" class="table table-bordered table-hover">
				...
				<tbody>
					<?php while($row = $query->fetch_assoc()){ $postID = $row['id']; $account = $con->query("select * from client_master where client_id = '".$row['act_no']."'")->fetch_object(); $prj_name = $con->query("select * from project_master where project_id = '".$row['prj_id']."'")->fetch_object();?>
						<tr>
							<td style="text-align:center;"><a href="sal_re_invoice.php?id=<?php echo $row['id']; ?>" ><button type="button" class="btn btn-default btn-sm">View</button></a> &nbsp;<a href="Process/delete.php?sal_re_invoice_del=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this Invoice ?');"><button type="button" class="btn btn-danger btn-sm">Delete</button></a></td>
							<td><?php echo $row['pr_no']; ?></td>
							<td><?php if(!empty($row['act_no'])){ echo $account->client_name; } ?></td>
							<td><?php if($row['prj_id'] != '0'){ echo $prj_name->project_name; } ?></td>
