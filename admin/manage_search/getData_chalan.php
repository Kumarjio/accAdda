...
if(isset($_POST['page'])){
	...
	else
	{
		...
		if(!empty($date)){
			...
			{
				$whereSQL .= "and s_date between '".$date1."' AND '".$date2."'";
			}
    ...
    //get number of rows
    $queryNum = $conn->query("SELECT COUNT(*) as postNum FROM quatation_mst where billbookno = '0' and c_id = '".$_SESSION['company_id']."' ".$whereSQL.$orderSQL);
    ...
    //get rows
    $query = $conn->query("SELECT * FROM quatation_mst where billbookno = '0' and c_id = '".$_SESSION['company_id']."' $whereSQL $orderSQL LIMIT $start,$limit");
			...
			<table id="" class="table table-bordered table-hover" style="margin-bottom:0px;">
				...
				<tbody>
					<?php while($row = $query->fetch_assoc()){ $postID = $row['s_id']; $account = $con->query("select * from client_master where client_id = '".$row['act_no']."'")->fetch_object(); $prj_name = $con->query("select * from project_master where project_id = '".$row['prj_id']."'")->fetch_object();?>
						<tr>
							<td style="text-align:center;"><a href="chalan_invoice.php?id=<?php echo $row['s_id']; ?>" ><button type="button" class="btn btn-default btn-sm">View</button></a> &nbsp;<a href="edit_chalan.php?id=<?php echo $row['s_id']; ?>"><button type="button" class="btn btn-info btn-sm">Edit</button></a> &nbsp;<a href="Process/delete.php?del_chalan_id=<?php echo $row['s_id']; ?>" onclick="return confirm('Are you sure you want to delete this Chalan ?');"><button type="button" class="btn btn-danger btn-sm">Delete</button></a></td>
							...
							<td><?php if($row['usage_flag'] == '1'){ echo "Yes"; } else{ echo "No"; }; ?></td>
